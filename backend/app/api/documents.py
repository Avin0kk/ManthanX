...
from app.schemas.document import ChunkResult, DocumentOut, DocumentUploadResponse
...
async def upload_document(
    ...
    try:
        document = await ingest_document(db, user.id, file.filename, content)
...
async def search_documents(
    ...
    results = await retrieve_relevant_chunks(db, user.id, query=q, top_k=top_k)
    return results
